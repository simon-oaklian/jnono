...
DEFAULT_EXAMS_V2 = [
    ...
    },
    {
        ...
        "mock_question_count": 115,
        "mock_time_limit_minutes": 210,
        "included_exam_codes": ["b_general_law_business"],
    },
]
...
DEFAULT_USERS = [
    ...
    },
    {
        ...
        "password": "CHANGE_ME_STUDENT_PASSWORD",
        "plan": "free",
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def require_user(self):
        ...
        with db_conn() as conn:
            ...
            row = conn.execute(
                """
                SELECT s.token, s.is_admin, u.id, u.name, u.email, u.plan, u.membership_tier, u.account_status
                FROM sessions s
    ...
    def require_admin(self):
        ...
        if not token:
            ...
            return None
        with db_conn() as conn:
            row = conn.execute("SELECT token, is_admin FROM sessions WHERE token = ?", (token,)).fetchone()
    ...
    def create_session(self, *, user_id: int | None, is_admin: int) -> str:
        ...
        with db_conn() as conn:
            conn.execute(
                "INSERT INTO sessions(token, user_id, is_admin, created_at) VALUES(?,?,?,?)",
                (token, user_id, is_admin, now_iso()),
            )
        return token
        ...
        try:
            ...
            if method == "POST" and path == "/api/auth/register":
                ...
                with db_conn() as conn:
                    ...
                    cur = conn.execute(
                        ...
                        VALUES(?,?,?,?,?,?,?,?)
                        """,
                        (name, email, password, "free", "free", created_at, 1, created_at),
                    )
            ...
            if method == "POST" and path == "/api/auth/login":
                ...
                with db_conn() as conn:
                    ensure_user_membership_columns(conn)
                    row = conn.execute(
                        "SELECT id, name, email, plan, account_status FROM users WHERE email=? AND password=?",
                        (email, password),
                    ).fetchone()
                if not row:
