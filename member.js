...
function shellEnsureControls() {
  ...
  try {
    ...
    if (_hdr && _zh && _ai && _zh.parentElement !== _hdr) {
      ...
      if (_exit) { _hdr.insertBefore(_zh, _exit); _hdr.insertBefore(_ai, _exit); }
      else { _hdr.appendChild(_zh); _hdr.appendChild(_ai); }
...
async function startCategoryQuiz(useWrongBook) {
  ...
  const selectedCategoryCode = String(topicSelect.value || "all").trim().toUpperCase();
...
function startMockExam() {
  ...
  state.quizQuestions = shuffle(baseQuestions).slice(0, actualCount);
  state.currentIndex = 0;
  state.answers = {};
...
function updateActionButtons() {
  ...
  wrongBookBtn.disabled = !hasExam || isMock;
}
...
(function () {
  ...
  window.hasChineseAccess = canChinese;
  window.hasExplanationAccess = canAi;
  window.hasMemoryTipsAccess = canAi;
  window.hasAiAnalysisAccess = canAi;
